...
<Card title="😤 Where Rust is Harder" icon="warning">
  ...
  <Card title="🤷 When Others Might Be Better" icon="question">
    ...
    **For simple CRUD apps**: Garbage collection overhead might be acceptable - **For teams new to
    systems programming**: Go's simplicity might be more approachable
  </Card>
</CardGrid>
...
<Tabs>
  <TabItem label="Performance Benefits">
    - **Memory usage**: 40% less memory than Java equivalent (no GC overhead) - **Latency**:
    Predictable performance - no GC pause spikes
  </TabItem>
