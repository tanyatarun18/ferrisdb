...
<Tabs>
  <TabItem label="Writer ✅">
    ...
    ```bash # See how data is written to disk cat ferrisdb-storage/src/sstable/writer.rs # Test
    SSTable creation cargo test -p ferrisdb-storage test_sstable_writer ``` Creates sorted files
    with index and footer.
  </TabItem>

  <TabItem label="Reader ❌">
    <Badge text="NOT IMPLEMENTED" variant="danger" />
    ```bash # See the skeleton cat ferrisdb-storage/src/sstable/reader.rs ``` Structure exists but
